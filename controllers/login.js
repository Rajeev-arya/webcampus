...
const verifyManager = async (req, res) => {
  try {
    ...
    const response = await MetaData.findOne({ "user.username": username })
      .select("_id user domain navigationbar collegename")
      .then((result) => {
        req.session.metadata = result;
        return result;
      })
    ...
    const college = response.user;

    if (college.password != password) {
      return res.status(401).json({ message: "Invalid Password" });
    }

    await Gallery.findOne({ domain: req.session.metadata.domain })
      .select("logo")
      .then((result) => {
    ...
    const token = jwt.sign(
      {
        ...
        domain: user.domain,
        navigationbar: user.navigationbar,
      },
      JWT_KEY
    );
