...
async function managerDashboard(req, res) {
  ...
  const response = await MainPage.findOne({ domain: domain })
    .select("notice news importantlink")
    .then((result) => {
      return result;
    })
    .catch((err) => {
  ...
  const news = response.news.content;
  const notice = response.notice.content;
  const importantlink = response.importantlink.content;

  res.render("manager/dashboard", { news, notice, importantlink });
}
...
// Header Link
const headerLinkPage = async (req, res) => {
  const domain = req.session.metadata.domain;

  const response = await MetaData.findOne({ domain: domain })
    .select("headerlink")
    .then((result) => {
      return result;
    })
  ...
  res.render("manager/widgets/add-headerlink", { data: response.headerlink });
  // res.json(response)
};
...
const importantLinkPage = async (req, res) => {
  ...
  res.render("manager/widgets/add-link", {
    data: response.importantlink.content,
  });
  // res.json(response)
};

const addHeaderLink = async (req, res) => {
  const body = req.body;
  const payload = {
    color: body.color,
    href: body.href,
  ...
  const result = await MetaData.findOneAndUpdate(
    { domain: domain },
    { $push: { headerlink: payload } },
    { new: true }
  );
  res.redirect("add-headerlink");
};
...
const addNavigationItem = async (req, res) => {
  ...
  await MetaData.findOne({ domain })
    .select("_id user domain navigationbar")
    .then((result) => {
      console.log(result);
...
const deleteSubNavitem = async (req, res) => {
  ...
  const deleteNavigationBarSubItem = async (domain, navitemid, subitemid) => {
    try {
      ...
      // Refresh Side Navigation Pages Section
      await MetaData.findOne({ domain })
        .select("user domain navigationbar")
        .then((result) => {
          console.log(result);
          req.session.metadata = result;
        })
...
const updateMainPageData = async (req, res) => {
  ...
  let themecolor = await AdminUser.findOne(
    { "theme.colourpallet": { $elemMatch: { name: colorNameToSearch } } },
  ...
  const updateMetaData = await MetaData.findByIdAndUpdate(
    collegeid,
    meta_payload
  );
